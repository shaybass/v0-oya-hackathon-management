...
export default function NewFoodSupplierPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      const { data, error } = await supabase.from("food_suppliers").insert([values]).select()

      if (error) throw error

      toast({
        title: "ספק מזון נוצר בהצלחה",
        description: "ספק המזון החדש נוסף למערכת",
      })

      router.push("/food/suppliers")
    } catch (error) {
      console.error("Error creating food supplier:", error)
      toast({
        title: "שגיאה",
        description: "אירעה שגיאה בעת יצירת ספק המזון",
        variant: "destructive",
      })
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle>הוספת ספק מזון חדש</CardTitle>
          <CardDescription>הוסף ספק מזון חדש למאגר הספקים</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="הזן את שם ספק המזון" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
